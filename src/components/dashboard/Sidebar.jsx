...
const Sidebar = () => {
  ...
  return (
    ...
    {isOpen&&<FaMinus />}
      ...
      <div className='flex cursor-pointer  space-x-2' > <IoIosApps  size={30}/> {isOpen&& <span className='hover:border-b-2 border-blue-400 cursor-pointer'> My applications</span>}</div>
      <div className='flex cursor-pointer space-x-2' > <IoIosCheckmarkCircle  size={30}/> {isOpen&& <span className='hover:border-b-2 border-blue-400 cursor-pointer'> Preference</span>}</div>
